...
class Matern32KernelUni(UnivariateKernel):
    ...
    def _evaluate(self, x1: np.ndarray, x2: np.ndarray) -> np.ndarray:
        ...
        for i in range(n1):
            for j in range(n2):
                abs_diff = np.sqrt(3) * abs(scaled_diff(x1[i], x2[j], self.ell, 1))
                K[i, j] = (1 + abs_diff) * np.exp(-abs_diff)
        return K


class Matern32Kernel(ProductKernel):
    def __init__(self, config: Optional[dict] = None):
        ...
        kernels = [Matern32KernelUni(ell=elli) for elli in ell]
